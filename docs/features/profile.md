...
- **Email is not self-editable.** It identifies the account and anchors OTP verification, so changing it needs its own re-verification flow rather than a profile save. Neither are `role`, `status` or `is_email_verified`: `status` is the admin-approval gate the [admin toggle](admin-user-management.md) flips, and nothing self-served may touch it. The Form Request whitelists the payload and `ProfileService::EDITABLE_FIELDS` whitelists it again, so a field added to the request later cannot silently become self-editable.
